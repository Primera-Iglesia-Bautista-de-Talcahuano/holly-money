    ...
    <section className="mx-auto max-w-6xl space-y-8">
      <Card className="bg-surface-container-lowest p-6 sm:p-8 shadow-[0px_20px_40px_-12px_rgba(25,28,30,0.08)]">
        ...
        <form className="mt-4 grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 items-end" method="get">
          ...
          <select name="tipo" defaultValue={tipo} className="h-10 w-full rounded-xl border-none bg-surface-container-low px-3 py-2 text-sm font-medium text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/30">
            <option value="ALL">Todos los tipos</option>
            <option value="INGRESO">Ingreso</option>
            <option value="EGRESO">Egreso</option>
          </select>
          <select name="estado" defaultValue={estado} className="h-10 w-full rounded-xl border-none bg-surface-container-low px-3 py-2 text-sm font-medium text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/30">
